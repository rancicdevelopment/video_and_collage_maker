...
enum _Sort { newestFirst, oldestFirst, nameAZ, nameZA }

class AllCollageProjectsScreen extends StatefulWidget {
  const AllCollageProjectsScreen({super.key});

  @override
  State<AllCollageProjectsScreen> createState() =>
      _AllCollageProjectsScreenState();
}

class _AllCollageProjectsScreenState extends State<AllCollageProjectsScreen> {
  static const _kPurple = Color(0xFF7B35C8);

  List<CollageDraft> _allDrafts = [];
  List<CollageDraft> _drafts = [];
  bool _loading = true;
  ...
  Future<void> _renameDraft(CollageDraft draft) async {
  ...
  Future<void> _deleteDraft(CollageDraft draft) async {
  ...
  Future<void> _deleteSelected() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Delete selected?',
            style: TextStyle(color: Colors.white)),
        content: Text(
          '$count collage${count > 1 ? 's' : ''} will be permanently deleted.',
          style: const TextStyle(color: Colors.white70),
        ),
        actions: [
          TextButton(
    ...
    if (confirm == true) {
      ...
      for (final id in ids) {
        await CollageDraftManager.instance.delete(id);
      }
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _deleteAll() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF2A2A2A),
        title: const Text('Delete all collages?',
            style: TextStyle(color: Colors.white)),
        content: Text(
          'All $count collage${count > 1 ? 's' : ''} will be permanently deleted.',
          style: const TextStyle(color: Colors.white70),
        ),
        actions: [
          TextButton(
    ...
    if (confirm == true) {
      final all = List<CollageDraft>.from(_allDrafts);
      for (final d in all) {
        await CollageDraftManager.instance.delete(d.id);
      }
      if (!mounted) return;
      setState(() {
        _allDrafts.clear();
  ...
  Widget build(BuildContext context) {
    final hasActive =
        (_sortOrder != _Sort.newestFirst || _searchQuery.isNotEmpty);
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (!_loading && _drafts.isNotEmpty)
            IconButton(
              ...
              icon: Icon(
                ...
                color: _selectMode ? _kPurple : Colors.white70,
              ),
              onPressed: () => setState(() {
          ...
          IconButton(
            ...
            icon: Icon(
              Icons.sort,
              color: hasActive ? _kPurple : Colors.white70,
            ),
  ...
  Widget _buildItem(CollageDraft draft) {
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        '${draft.cells.where((c) => c.filePath != null).length}/${draft.cells.length} cells',
                        style: const TextStyle(
  ...
  Widget _buildSelectBar() {
    final allSelected =
        _selected.length == _drafts.length && _drafts.isNotEmpty;
    return Container(
  ...
  Widget _buildThumbnail(CollageDraft draft) {
    ...
    // Try first image cell as thumbnail.
    for (final cell in draft.cells) {
      if (cell.filePath == null || cell.isVideo) continue;
      final f = File(cell.filePath!);
                ...
                ListTile(
                  ...
                  trailing: _selected == sort
                      ? const Icon(Icons.check,
                          color: Color(0xFF7B35C8), size: 18)
                      : null,
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    style: ElevatedButton.styleFrom(
                      backgroundColor: const Color(0xFF7B35C8),
                      foregroundColor: Colors.white,
                    ...
                    onPressed: () =>
                        widget.onApply(_selected, _searchCtrl.text.trim()),
